...
#      GitOK2.xcodeproj），因此不能用 agvtool new-marketing-version（它会
#      同时改所有 target，污染 Lumi 的 5.8.4 等版本）。这里只修改
#      BookletMaker 独立的 xcconfig。
...
if ! echo "$BUILD_NUMBER" | grep -qE '^[1-9][0-9]*$'; then
  echo "==> [set-booklet-version] 错误：CI_BUILD_NUMBER='$BUILD_NUMBER' 不是正整数" >&2
  exit 1
fi
