#!/bin/bash
#
# set-databasemanager-version.sh — 把 DatabaseManager 的版本号写进工程文件
#
# 设计要点：
#   1. 版本号来源是 git tag（由 GitHub Actions 的 databasemanager-tag.yml 推上来），
#      通过 Xcode Cloud 注入的 CI_TAG 环境变量读取，格式 databasemanager-v1.2.3。
#      版本号不进 git 历史，只改 CI 临时 checkout 的工程副本。
#   2. 单工程多 app 结构（共用一个 GitOK2.xcodeproj），因此不能用 agvtool
#      new-marketing-version（会同时改所有 target）。这里精确改 DatabaseManager
#      的 xcconfig。
...
#
# Usage: set-databasemanager-version.sh
#   依赖环境变量：
#     CI_TAG          Xcode Cloud 注入的触发 tag（databasemanager-v1.2.3）
...
TAG="${CI_TAG:-}"
if [ -z "$TAG" ]; then
  echo "==> [set-databasemanager-version] CI_TAG 未设置，跳过版本写入（非 tag 触发的构建）。"
  exit 0
fi

# 只认 databasemanager-v* 前缀的 tag；其他 tag 直接跳过，
# 避免在其他 app 发版时误改 DatabaseManager 版本。
case "$TAG" in
  databasemanager-v*) ;;
  *)
    echo "==> [set-databasemanager-version] CI_TAG='$TAG' 不是 databasemanager-v*，跳过。"
    exit 0
    ;;
esac

VERSION="${TAG#databasemanager-v}"
# 校验是 x.y.z 形式，防止脏 tag 写进工程文件
if ! echo "$VERSION" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+$'; then
  echo "==> [set-databasemanager-version] 错误：'$TAG' 解析出的版本 '$VERSION' 不是 x.y.z" >&2
  exit 1
fi

BUILD_NUMBER="${CI_BUILD_NUMBER:-}"
if ! echo "$BUILD_NUMBER" | grep -qE '^[1-9][0-9]*$'; then
  echo "==> [set-databasemanager-version] 错误：CI_BUILD_NUMBER='$BUILD_NUMBER' 不是正整数" >&2
  exit 1
fi

echo "==> [set-databasemanager-version] 写入 DatabaseManager 版本"
echo "    tag:              $TAG"
...
if ! grep -q "^MARKETING_VERSION = $VERSION$" "$XCCONFIG"; then
  echo "==> [set-databasemanager-version] 错误：MARKETING_VERSION 未写入 $XCCONFIG" >&2
  exit 1
fi
if ! grep -q "^CURRENT_PROJECT_VERSION = $BUILD_NUMBER$" "$XCCONFIG"; then
  echo "==> [set-databasemanager-version] 错误：CURRENT_PROJECT_VERSION 未写入 $XCCONFIG" >&2
  exit 1
fi
