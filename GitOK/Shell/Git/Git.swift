...
class Git {
    ...
    static func push(_ path: String, debugPrint: Bool = false) throws -> String {
        try Git.run("push --porcelain", path: path, debugPrint: debugPrint)
    }
    ...
    static func diff(_ path: String, debugPrint: Bool = false) throws -> String {
        try Git.run("diff", path: path, debugPrint: debugPrint)
    }
