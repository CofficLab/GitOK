@testable import GitAutoPushPlugin
import Combine
import Foundation
import Testing

@Suite("GitAutoPushPlugin")
struct GitAutoPushPluginTests {
    @Test("metadata matches legacy plugin identity")
    func metadata() {
        #expect(GitAutoPushPlugin.metadata.id == "GitAutoPushPlugin")
        #expect(GitAutoPushPlugin.metadata.iconName == "arrow.up.circle")
        #expect(GitAutoPushPlugin.metadata.tableName == "Localizable")
    }

    @Test("localization catalog is packaged")
    func localizationCatalog() {
        #expect(GitAutoPushPluginLocalization.bundle.url(forResource: "Localizable", withExtension: "xcstrings") != nil)
        #expect(GitAutoPushPluginLocalization.string("Auto Push").isEmpty == false)
    }
}
