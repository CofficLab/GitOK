...
public struct RemoteRepositoryView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text(RemoteRepositoryPluginLocalization.string("Remote Repository Management"))
                    .font(.title2)
                    .fontWeight(.medium)
            ...
            Text(RemoteRepositoryPluginLocalization.string("Manage Git remote repository configuration for the current project"))
                .font(.caption)
                .foregroundColor(.secondary)
        }
...
private extension RemoteRepositoryView {
    ...
    func publishCurrentBranch() {
        guard let remote = preferredPublishRemote else {
            errorMessage = RemoteRepositoryPluginLocalization.string("Please add a remote repository first")
            return
        }
            ...
            do {
                ...
                guard let branch = try repository.currentBranchName() else {
                    throw NSError(
                        ...
                        code: -1,
                        userInfo: [NSLocalizedDescriptionKey: RemoteRepositoryPluginLocalization.string("Current repository has no publishable branch")]
                    )
                }
                ...
                await MainActor.run {
                    ...
                    postRemoteActionMessage = String(format: RemoteRepositoryPluginLocalization.string("Published current branch %@, and set upstream to %@/%@."), branch, remote.name, branch)
                    loadRemotes()
                }
            } catch {
                await MainActor.run {
                    isPublishingCurrentBranch = false
                    errorMessage = String(format: RemoteRepositoryPluginLocalization.string("Failed to publish current branch: %@"), error.localizedDescription)
                }
            }
        }
    }
