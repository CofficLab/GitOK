...
struct AddRemoteSheet: View {
    ...
    var body: some View {
        RemoteForm(
            title: RemoteRepositoryPluginLocalization.string("Add Remote Repository"),
            subtitle: RemoteRepositoryPluginLocalization.string("Add a new Git remote repository"),
            remoteName: $remoteName,
            remoteURL: $remoteURL,
            errorMessage: errorMessage,
            primaryTitle: RemoteRepositoryPluginLocalization.string("Add"),
    ...
    private func addRemote() {
        ...
        guard input.name.isEmpty == false, input.url.isEmpty == false else {
            errorMessage = RemoteRepositoryPluginLocalization.string("Please fill in all fields")
            return
        }
...
struct EditRemoteSheet: View {
    ...
    var body: some View {
        RemoteForm(
            title: RemoteRepositoryPluginLocalization.string("Edit Remote Repository"),
            subtitle: RemoteRepositoryPluginLocalization.string("Modify the remote repository's name and URL"),
            remoteName: $remoteName,
            remoteURL: $remoteURL,
            errorMessage: errorMessage,
            primaryTitle: RemoteRepositoryPluginLocalization.string("Save"),
    ...
    private func saveRemote() {
        let input = RemoteRepositoryFormRules.normalizedInput(name: remoteName, url: remoteURL)
        guard input.name.isEmpty == false, input.url.isEmpty == false else {
            errorMessage = RemoteRepositoryPluginLocalization.string("Please fill in all required information")
            return
        }
...
private struct RemoteForm: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 16) {
                formField(
                    title: RemoteRepositoryPluginLocalization.string("Remote Name"),
                    placeholder: RemoteRepositoryPluginLocalization.string("e.g. origin"),
                    text: $remoteName
                )

                formField(
                    title: RemoteRepositoryPluginLocalization.string("Remote URL"),
                    placeholder: RemoteRepositoryPluginLocalization.string("e.g. https://github.com/user/repo.git"),
                    text: $remoteURL
                )
            }
