...
public struct BranchManagementView: View {
    ...
    public init(context: GitBranchPluginContext) {
    ...
    public var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                AppSearchBar(
                    text: $searchText,
                    placeholder: GitBranchPluginLocalization.string("Search branches")
                )
                branchListSection
    ...
    private var newBranchSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(GitBranchPluginLocalization.string("New Branch"))
                .font(.headline)
            HStack(spacing: 8) {
                AppInputField(
                    GitBranchPluginLocalization.string("Branch name"),
                    text: $newBranchName
                )
                AppIconButton(
                    systemImage: "plus",
                    label: GitBranchPluginLocalization.string("New Branch"),
                    tint: .accentColor
                ) {
                    createBranch()
                }
    ...
    private var branchListSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(GitBranchPluginLocalization.string("Switch Branch"))
                .font(.headline)
            if isLoading {
                AppLoadingOverlay(message: GitBranchPluginLocalization.string("Loading branches..."))
                    .frame(maxWidth: .infinity, minHeight: 60)
            } else if filteredBranches.isEmpty {
                AppEmptyState(
                    icon: "arrow.triangle.branch",
                    title: GitBranchPluginLocalization.string("No branches yet")
                )
                    .frame(maxWidth: .infinity, minHeight: 60)
            } else {
    ...
    private var remoteBranchesSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(GitBranchPluginLocalization.string("Remote Branches"))
                .font(.headline)
            ForEach(filteredRemoteBranches, id: \.self) { branchName in
                HStack {
                    ...
                    .help(GitBranchPluginLocalization.string("Delete Remote Branch"))
                }
    ...
    private var compareSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(GitBranchPluginLocalization.string("Branch Compare"))
                .font(.headline)
            Picker(GitBranchPluginLocalization.string("Base"), selection: $compareBaseBranch) {
                ForEach(branches) { branch in
                    Text(branch.name).tag(branch as GitBranchSummary?)
                }
            }
            Picker(GitBranchPluginLocalization.string("Head"), selection: $compareHeadBranch) {
                ForEach(branches) { branch in
                    Text(branch.name).tag(branch as GitBranchSummary?)
                }
            }
            AppButton(
                GitBranchPluginLocalization.string("Compare"),
            ...
            if isComparing {
                AppLoadingOverlay(message: GitBranchPluginLocalization.string("Comparing branches..."), size: .small)
                ...
                Text(GitBranchPluginLocalization.string("Select base/head to view ahead/behind counts, commits, and file changes."))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
        }
    ...
    private func compareResultView(_ compare: GitBranchCompare) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 12) {
                Text(String(format: GitBranchPluginLocalization.string("Ahead %d"), compare.ahead))
                    .font(.caption)
                    .foregroundColor(.green)
                Text(String(format: GitBranchPluginLocalization.string("Behind %d"), compare.behind))
                    .font(.caption)
                    .foregroundColor(.orange)
                Text(String(format: GitBranchPluginLocalization.string("%d files"), compare.files.count))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            ...
            HStack {
                AppButton(
                    GitBranchPluginLocalization.string("Merge Head into Base"),
    ...
    private func pullRequestButton(compare: GitBranchCompare) -> some View {
        if let links = BranchLogic.pullRequestLinks(
            ...
            AppButton(
                GitBranchPluginLocalization.string("Create PR"),
...
private extension BranchManagementView {
    ...
    func deleteBranch(_ branch: GitBranchSummary) {
        guard selectedBranch?.id != branch.id else {
            errorMessage = GitBranchPluginLocalization.string("Cannot delete the current branch")
            return
        }
        let branchName = branch.name
        runAction { service in
    ...
    func renameSheet(_ branch: GitBranchSummary) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            Text(GitBranchPluginLocalization.string("Rename Branch"))
                .font(.headline)
            Text(branch.name)
                .font(.caption)
                .foregroundColor(.secondary)
            AppInputField(
                GitBranchPluginLocalization.string("New branch name"),
                text: $renameBranchName
            )
            HStack(spacing: 8) {
                Spacer()
                AppButton(
                    GitBranchPluginLocalization.string("Cancel"),
                    style: .secondary,
                    size: .small
                ) {
                    branchToRename = nil
                ...
                AppButton(
                    GitBranchPluginLocalization.string("Rename"),
                    systemImage: "pencil",
                    style: .primary,
                    size: .small
                ) {
    ...
    func upstreamSheet(_ branch: GitBranchSummary) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            Text(GitBranchPluginLocalization.string("Set Upstream Branch"))
                .font(.headline)
            Text(branch.name)
                .font(.caption)
                .foregroundColor(.secondary)
            Picker(GitBranchPluginLocalization.string("Upstream"), selection: $selectedUpstreamBranch) {
            ...
            HStack(spacing: 8) {
                ...
                AppButton(
                    GitBranchPluginLocalization.string("Set Upstream"),
                    systemImage: "link",
                    style: .primary,
                    size: .small
                ) {
