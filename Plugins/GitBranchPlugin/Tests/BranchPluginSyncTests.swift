...
//
// GitBranchPluginContext is a value-type snapshot. The fix ensures that:
...
@Suite("GitBranchPlugin Sync Tests")
struct GitBranchPluginSyncTests {

    // MARK: - Context is a value-type snapshot (immutable after creation)

    @Test("GitBranchPluginContext is a value-type snapshot")
    func contextIsValueTypeSnapshot() {
        ...
        let newCtx = GitBranchPluginContext(projectURL: url, branchName: "develop", isGitRepository: true)
        #expect(ctx.branchName == "main")
    ...
    @MainActor
    @Test("New GitBranchPluginContext correctly captures updated branchName from GitOKPluginContext")
    func newContextCapturesUpdatedBranch() {
        ...
        let oldCtx = GitBranchPluginContext(oldPluginCtx)
        let newCtx = GitBranchPluginContext(newPluginCtx)
    ...
    @MainActor
    @Test("GitBranchPlugin.statusBarLeadingItems produces views with correct context")
    func statusBarLeadingItemsProducesCorrectContext() {
        ...
        let items1 = GitBranchPlugin.statusBarLeadingItems(context: ctx1)
        #expect(items1.count == 1)

        let ctx2 = GitOKPluginContext(projectURL: url, branchName: "develop", isGitRepository: true)
        let items2 = GitBranchPlugin.statusBarLeadingItems(context: ctx2)
        #expect(items2.count == 1)
    }

    @MainActor
    @Test("GitBranchPlugin.toolbarTrailingItems produces views with correct context")
    func toolbarTrailingItemsProducesCorrectContext() {
        let url = URL(fileURLWithPath: "/tmp/repo")

        let ctx1 = GitOKPluginContext(projectURL: url, branchName: "main", isGitRepository: true)
        let items1 = GitBranchPlugin.toolbarTrailingItems(context: ctx1)
        #expect(items1.count == 1)

        let ctx2 = GitOKPluginContext(projectURL: url, branchName: "feature", isGitRepository: true)
        let items2 = GitBranchPlugin.toolbarTrailingItems(context: ctx2)
        #expect(items2.count == 1)
    }
    ...
    func toolBarContextMappingBothStates() {
        ...
        let mainCtx = GitBranchPlugin.toolBarContext(from: GitOKPluginContext(
            projectURL: url, branchName: "main", isGitRepository: true))
        let developCtx = GitBranchPlugin.toolBarContext(from: GitOKPluginContext(
    ...
    func rapidSwitchIndependentSnapshots() {
        ...
        let contexts = branches.map { branch in
            GitBranchPluginContext(projectURL: url, branchName: branch, isGitRepository: true)
        }
