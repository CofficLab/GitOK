...
//
// ## Bug Description
// When a user performs a merge (e.g., from dev to main), the GitBranchPlugin toolbar
// continues to show the old branch name (dev) even though HEAD has changed to main.
//
// ## Root Cause
// The bug is NOT in GitBranchPlugin itself - it's in the data flow:
...
struct BranchMergeRefreshBugTests {
    ...
    func branchMonitorUsesFileSystemMonitoring() async throws {
        // With the refactor, GitBranchPlugin now uses BranchMonitor to watch .git/HEAD directly.
        // This is more reliable and immediate than relying on NotificationCenter.
        let branchMonitorSourcePath = "/Users/angel/Code/Coffic/GitOK/Plugins/GitBranchPlugin/Sources/Services/BranchMonitor.swift"
