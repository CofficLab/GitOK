...
public struct ConflictResolverRow: View {
    ...
    public var body: some View {
        HStack(spacing: DesignTokens.Spacing.md) {
                ...
                HStack(spacing: DesignTokens.Spacing.xs) {
                    rowAction(icon: "arrow.up.right.square", help: ConflictResolverPluginLocalization.string("Open file"), action: onOpen)
                    rowAction(icon: "folder", help: ConflictResolverPluginLocalization.string("Reveal in Finder"), action: onReveal)

                    if let onStage {
                        rowAction(
                            icon: file.state == .unresolved ? "checkmark.circle" : "square.and.arrow.down",
                            tint: file.state.tint,
                            help: file.state == .unresolved ? ConflictResolverPluginLocalization.string("Mark as resolved and stage") : ConflictResolverPluginLocalization.string("Stage resolved file"),
                            action: onStage
                        )
                        .disabled(isBusy)
                    ...
                    if isSelected {
                        Text(ConflictResolverPluginLocalization.string("Selected"))
                            .font(DesignTokens.Typography.caption1)
...
private extension GitMergeFileState {
    var title: String {
        switch self {
        case .unresolved:
            return ConflictResolverPluginLocalization.string("Unresolved")
        case .pendingStage:
            return ConflictResolverPluginLocalization.string("Pending Stage")
        case .staged:
            return ConflictResolverPluginLocalization.string("Staged")
        }
    }
