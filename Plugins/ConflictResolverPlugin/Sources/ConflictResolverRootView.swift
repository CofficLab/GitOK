            ...
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.lg) {
                HStack(alignment: .top, spacing: DesignTokens.Spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: DesignTokens.Spacing.xs) {
                        Text(ConflictResolverPluginLocalization.string("Merge paused by conflicts"))
...
private struct ConflictResolverRootStatus: Equatable {
    ...
    var message: String {
        if conflictCount > 0 {
            return String(
                format: ConflictResolverPluginLocalization.string("Resolve %d conflicted files before using the rest of GitOK."),
                conflictCount
            )
        }

        return ConflictResolverPluginLocalization.string("Resolve the merge state before using the rest of GitOK.")
    }
}
