...
public struct ConflictResolverList: View {
        ...
        VStack(alignment: .leading, spacing: DesignTokens.Spacing.md) {
                HStack(alignment: .top, spacing: DesignTokens.Spacing.md) {
                    VStack(alignment: .leading, spacing: DesignTokens.Spacing.xs) {
                        Text(ConflictResolverPluginLocalization.string("Conflict Resolution"))
                ...
                HStack(spacing: DesignTokens.Spacing.sm) {
                    Label(
                        isMerging ? "\(mergeFiles.count) \(ConflictResolverPluginLocalization.string("Merge Files"))" : ConflictResolverPluginLocalization.string("Working Tree Ready"),
    ...
    private var conflictListView: some View {
        VStack(alignment: .leading, spacing: DesignTokens.Spacing.md) {
                HStack {
                    Label(ConflictResolverPluginLocalization.string("Merge Files"), systemImage: "exclamationmark.triangle")
                        .font(DesignTokens.Typography.bodyEmphasized)
                    Spacer()
                }
            ...
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.md) {
                HStack(alignment: .top, spacing: DesignTokens.Spacing.md) {
                    ...
                    HStack(spacing: DesignTokens.Spacing.xs) {
                        smallToolButton(icon: "square.and.pencil", title: ConflictResolverPluginLocalization.string("Edit"), help: ConflictResolverPluginLocalization.string("Open the conflicted file in the default editor")) {
                            openFile(selectedFile)
                        }
                        smallToolButton(icon: "doc.text", title: "Base", help: ConflictResolverPluginLocalization.string("Open base version")) {
                            openConflictVersion(.base, path: selectedFile)
                        }
                        smallToolButton(icon: "arrow.left.square", title: "Ours", help: ConflictResolverPluginLocalization.string("Open ours version")) {
                            openConflictVersion(.ours, path: selectedFile)
                        }
                        smallToolButton(icon: "arrow.right.square", title: "Theirs", help: ConflictResolverPluginLocalization.string("Open theirs version")) {
    ...
    private var workflowHint: String {
        ...
        case .unresolved:
            return ConflictResolverPluginLocalization.string("Edit the file or accept one side, then mark as resolved.")
        case .pendingStage:
            return ConflictResolverPluginLocalization.string("Conflict markers cleared, stage to continue merge.")
        case .staged:
            return ConflictResolverPluginLocalization.string("File staged, waiting for all files to complete before continuing merge.")
        case nil:
            return ""
        }
    }
    ...
    private func diffPreview(text: String) -> some View {
        ScrollView([.horizontal, .vertical]) {
            Text(text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? ConflictResolverPluginLocalization.string("No conflict diff to display.") : text)
...
private extension ConflictResolverList {
    ...
    func showError(_ error: Error) {
        let alert = NSAlert()
        alert.messageText = ConflictResolverPluginLocalization.string("Conflict operation failed")
