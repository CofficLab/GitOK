    ...
    public var body: some View {
        AppSettingsRow(isSelected: isSelected, verticalPadding: 12) {
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        ...
                        if isCurrentUpstreamRemote {
                            AppTag(
                                GitRemoteRepositoryPluginLocalization.string("Upstream"),
                ...
                HStack(spacing: 8) {
                    if let webLink {
                        iconButton("safari", help: GitRemoteRepositoryPluginLocalization.string("Open Remote Page")) {
                            NSWorkspace.shared.open(webLink.url)
                        }
                    }

                    iconButton("doc.on.doc", help: GitRemoteRepositoryPluginLocalization.string("Copy Remote URL")) {
                        NSPasteboard.general.clearContents()
                        NSPasteboard.general.setString(remote.url, forType: .string)
                    }

                    iconButton("pencil", help: GitRemoteRepositoryPluginLocalization.string("Edit")) {
                        onEdit(remote)
                    }

                    iconButton("trash", tint: .red, help: GitRemoteRepositoryPluginLocalization.string("Delete")) {
        ...
        .alert(GitRemoteRepositoryPluginLocalization.string("Confirm Delete"), isPresented: $showDeleteAlert) {
            Button(GitRemoteRepositoryPluginLocalization.string("Cancel"), role: .cancel) {}
            Button(GitRemoteRepositoryPluginLocalization.string("Delete"), role: .destructive) {
                onDelete(remote)
            }
        } message: {
