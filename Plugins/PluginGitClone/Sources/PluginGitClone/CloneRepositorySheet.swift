...
public struct CloneRepositorySheet: View {
    ...
    private var validationMessage: String? {
        if trimmedRemoteURL.isEmpty {
            return PluginGitCloneLocalization.string("Please enter a remote repository URL")
        }
        ...
        guard destinationURL != nil else {
            return PluginGitCloneLocalization.string("Invalid destination path")
        }
...
extension CloneRepositorySheet {
    var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(PluginGitCloneLocalization.string("Clone Repository"))
                .font(.title2)
                .fontWeight(.semibold)

            Text(PluginGitCloneLocalization.string("Clone a Git repository from a remote URL and automatically add it to the project list."))
                .font(.caption)
    ...
    var remoteSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(PluginGitCloneLocalization.string("Remote Repository"))
                .font(.headline)

            TextField("https://github.com/owner/repo.git", text: $remoteURL)
                .textFieldStyle(.roundedBorder)

            Text(PluginGitCloneLocalization.string("Repository Name"))
                .font(.caption)
                .foregroundColor(.secondary)

            TextField(PluginGitCloneLocalization.string("Repository Name"), text: $repositoryName)
                .textFieldStyle(.roundedBorder)
            ...
            if normalizedRemoteURL != trimmedRemoteURL, trimmedRemoteURL.isEmpty == false {
                Text(String(format: PluginGitCloneLocalization.string("Will use: %@"), normalizedRemoteURL))
                    .font(.caption)
                    .foregroundColor(.secondary)
    ...
    var githubAccountSection: some View {
        DisclosureGroup {
            VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: 8) {
                    TextField(PluginGitCloneLocalization.string("github.com or ghe.example.com"), text: $githubHost)
                        .textFieldStyle(.roundedBorder)

                    TextField(PluginGitCloneLocalization.string("Username"), text: $githubUsername)
                        .textFieldStyle(.roundedBorder)
                }

                SecureField(PluginGitCloneLocalization.string("Personal Access Token"), text: $githubToken)
                    .textFieldStyle(.roundedBorder)
                ...
                HStack {
                    Button {
                        ...
                        if isLoadingGitHubRepositories {
                            ...
                            Text(PluginGitCloneLocalization.string("Connect and List Repositories"))
                                .frame(minWidth: 110)
                        }
                    }
                    ...
                    Button(PluginGitCloneLocalization.string("Create Token")) {
                ...
                if let githubErrorMessage {
                    ...
                    Text(PluginGitCloneLocalization.string("Token will be saved via the current Git credential helper. Repository list comes from GitHub/GitHub Enterprise API."))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }

                if githubRepositories.isEmpty == false {
                    TextField(PluginGitCloneLocalization.string("Search Repositories"), text: $githubSearchText)
                        .textFieldStyle(.roundedBorder)
            ...
            Label(PluginGitCloneLocalization.string("GitHub / Enterprise Account Clone"), systemImage: "person.crop.circle.badge.plus")
                .font(.headline)
        }
    }
    ...
    var destinationSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(PluginGitCloneLocalization.string("Local Path"))
                .font(.headline)
            ...
            HStack {
                ...
                Button(PluginGitCloneLocalization.string("Choose Directory")) {
            ...
            if let destinationURL {
                VStack(alignment: .leading, spacing: 4) {
                    Text(PluginGitCloneLocalization.string("Final Path"))
                        .font(.caption)
                        .foregroundColor(.secondary)
            ...
            if destinationState == .existingEmptyDirectory {
                Text(PluginGitCloneLocalization.string("Target directory exists but is empty. Cloning directly into it is allowed."))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
        }
    ...
    var statusSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            if let validationMessage {
                ...
                Text(PluginGitCloneLocalization.string("Ready to Clone"))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            ...
            if let errorMessage, errorMessage.isEmpty == false {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if credentialHost != nil {
                        Button(PluginGitCloneLocalization.string("Enter Credentials and Retry")) {
                    ...
                    if sshHelpRemoteURL != nil {
                        Button(PluginGitCloneLocalization.string("View SSH Handling")) {
    ...
    var credentialRetrySheet: some View {
        VStack(alignment: .leading, spacing: 18) {
            VStack(alignment: .leading, spacing: 6) {
                Label(PluginGitCloneLocalization.string("Git Credentials Required"), systemImage: "key.fill")
                    .font(.title3)
                    .fontWeight(.semibold)

                Text(PluginGitCloneLocalization.string("After saving HTTPS credentials for %@, GitOK will automatically retry the clone. Credentials will be saved via the current Git credential helper.").replacingOccurrences(of: "%@", with: credentialHost ?? PluginGitCloneLocalization.string("remote server")))
                    .font(.caption)
                    .foregroundColor(.secondary)
            ...
            VStack(alignment: .leading, spacing: 10) {
                TextField(PluginGitCloneLocalization.string("Username"), text: $credentialUsername)
                    .textFieldStyle(.roundedBorder)
                    .disabled(isSavingCredential)

                SecureField(PluginGitCloneLocalization.string("Personal Access Token or Password"), text: $credentialToken)
                    .textFieldStyle(.roundedBorder)
                    .disabled(isSavingCredential)
            ...
            HStack {
                ...
                Button {
                    ...
                    if isSavingCredential {
                        ...
                        Text(PluginGitCloneLocalization.string("Save and Retry"))
                            .frame(minWidth: 96)
                    }
                }
        ...
        HStack {
            ...
            Button {
                ...
                if isCloning {
                    ...
                    Text(PluginGitCloneLocalization.string("Clone"))
                        .frame(minWidth: 80)
                }
            }
            .buttonStyle(.borderedProminent)
    ...
    func cloneRepository() {
        ...
        errorMessage = nil
        cloneProgressMessage = PluginGitCloneLocalization.string("Ready to Clone")
        isCloning = true
        credentialHost = nil
        ...
        Task { @MainActor in
            setActivityStatus(PluginGitCloneLocalization.string("Cloning..."))
        }
            ...
            do {
                ...
                await MainActor.run {
                    if onCloneCompleted(destinationURL) {
                        ...
                        onCloneSucceeded(PluginGitCloneLocalization.string("Cloned %@").replacingOccurrences(of: "%@", with: repositoryName))
                        dismiss()
                    } else {
                        setActivityStatus(nil)
                        isCloning = false
                        cloneProgressMessage = nil
                        errorMessage = PluginGitCloneLocalization.string("Repository cloned locally, but failed to add to project list: %@").replacingOccurrences(of: "%@", with: destinationURL.path)
                    }
                }
            } catch {
                await MainActor.run {
        ...
        Task.detached(priority: .userInitiated) {
            do {
                ...
                await MainActor.run {
                    ...
                    githubToken = ""
                    githubErrorMessage = repositories.isEmpty ? PluginGitCloneLocalization.string("No accessible repositories found. Please verify that the token permissions include repo/read:org or corresponding Enterprise permissions.") : nil
                }
            } catch {
                await MainActor.run {
                    isLoadingGitHubRepositories = false
...
private enum GitHubCloneAPI {
    static func repositories(host: String, token: String) async throws -> [GitHubCloneRepository] {
        ...
        guard let url = components?.url else {
            throw NSError(domain: "GitOK.GitHubClone", code: -1, userInfo: [
                NSLocalizedDescriptionKey: PluginGitCloneLocalization.string("GitHub API URL is invalid")
            ])
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw NSError(domain: "GitOK.GitHubClone", code: -1, userInfo: [
                NSLocalizedDescriptionKey: PluginGitCloneLocalization.string("GitHub API did not return a valid response")
            ])
        }
    ...
    private static func apiErrorMessage(statusCode: Int, data: Data) -> String {
        if let payload = try? JSONDecoder().decode(GitHubAPIError.self, from: data) {
            return String(format: PluginGitCloneLocalization.string("GitHub API request failed (%lld): %@"), statusCode, payload.message)
        }
        return String(format: PluginGitCloneLocalization.string("GitHub API request failed (%lld)"), statusCode)
    }
