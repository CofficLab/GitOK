...
struct AutoPushConfigView: View {
    ...
    private func handleToggle(project: AutoPushProjectSnapshot, enabled: Bool) {
        ...
        showStatusMessage(String(format: AutoPushPluginLocalization.string("Auto-push %@: %@/%@"), enabled ? "enabled" : "disabled", project.projectTitle, branchName))
    }
    ...
    private func handleToggleConfig(_ config: ProjectBranchAutoPushConfig) {
        ...
        showStatusMessage(String(format: AutoPushPluginLocalization.string("Auto-push %@: %@/%@"), newStatus ? "enabled" : "disabled", config.projectTitle, config.branchName))
    }
    ...
    private func handleDeleteConfig(_ config: ProjectBranchAutoPushConfig) {
        ...
        showStatusMessage(String(format: AutoPushPluginLocalization.string("Configuration deleted: %@/%@"), config.projectTitle, config.branchName))
    }
    ...
    private func performPush(projectPath: String, branchName: String) {
        isLoading = true
        statusMessage = AutoPushPluginLocalization.string("Pushing...")
        ...
        Task {
            ...
            case .success:
                statusMessage = AutoPushPluginLocalization.string("Push succeeded")
            case let .failed(message):
                statusMessage = String(format: AutoPushPluginLocalization.string("Push failed: %@"), message)
            case .idle, .pushing:
                statusMessage = nil
            }
        }
