...
public struct SubmoduleStatusTile: View {
    ...
    private var content: some View {
        if isLoading {
            ...
            Text(GitSubmodulePluginLocalization.string("Submodule"))
                .foregroundStyle(.secondary)
        } else if issueCount > 0 {
            Text("\(GitSubmodulePluginLocalization.string("Submodule")) \(issueCount)")
                .font(.footnote.weight(.medium))
                .foregroundStyle(.orange)
                .monospacedDigit()
        } else {
            Text("\(GitSubmodulePluginLocalization.string("Submodule")) \(submodules.count)")
                .font(.footnote.weight(.medium))
                .foregroundStyle(.secondary)
                .monospacedDigit()
        }
    ...
    private var helpText: String {
        if submodules.isEmpty {
            return GitSubmodulePluginLocalization.string("No submodules in this repository")
        }
        if issueCount > 0 {
            return GitSubmodulePluginLocalization.string("Found %lld submodules that need attention", issueCount)
        }
        return GitSubmodulePluginLocalization.string("%lld submodules total", submodules.count)
    }
    ...
    private var header: some View {
        HStack(spacing: 10) {
            ...
            Spacer()
            AppButton(GitSubmodulePluginLocalization.string("Initialize All"), systemImage: "square.and.arrow.down", style: .secondary, size: .small) {
                initializeSubmodules()
            }
            .disabled(submodules.isEmpty)

            AppButton(GitSubmodulePluginLocalization.string("Update All"), systemImage: "arrow.triangle.2.circlepath", style: .secondary, size: .small) {
                updateSubmodules()
            }
            .disabled(submodules.isEmpty)
        }
    ...
    private var headerSubtitle: String {
        if submodules.isEmpty {
            return GitSubmodulePluginLocalization.string("No submodules found")
        }
        if issueCount > 0 {
            return GitSubmodulePluginLocalization.string("%lld submodules, %lld need attention", submodules.count, issueCount)
        }
        return GitSubmodulePluginLocalization.string("%lld submodules are up to date", submodules.count)
    }

    private var emptyState: some View {
        AppEmptyState(
            icon: "shippingbox",
            title: GitSubmodulePluginLocalization.string("Submodule"),
            description: GitSubmodulePluginLocalization.string("No Git submodules configured in this repository.")
        )
        .frame(minHeight: 160)
    }
    ...
    private func submoduleRow(_ submodule: GitRepositoryCLI.GitSubmodule) -> some View {
        AppSettingsRow(verticalPadding: 8) {
            HStack(alignment: .top, spacing: 8) {
                ...
                AppButton(GitSubmodulePluginLocalization.string("Diff"), systemImage: "doc.text.magnifyingglass", style: .tonal, size: .small) {
                    loadDiff(for: submodule.path)
                }

                AppButton(
                    submodule.status == .uninitialized ? GitSubmodulePluginLocalization.string("Initialize") : GitSubmodulePluginLocalization.string("Update"),
    ...
    private func diffSection(path: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text(GitSubmodulePluginLocalization.string("Diff: %@", path))
                    .font(.subheadline.weight(.semibold))
                    .lineLimit(1)
                    .truncationMode(.middle)
                Spacer()
                AppButton(GitSubmodulePluginLocalization.string("Close"), systemImage: "xmark", style: .ghost, size: .small) {
                    diffPath = nil
                    diffText = nil
                }
            }
            ...
            if isDiffLoading {
                AppLoadingOverlay(message: GitSubmodulePluginLocalization.string("Loading..."), size: .small)
                    .frame(height: 60)
                ...
                Text(GitSubmodulePluginLocalization.string("No diff summary available for this submodule."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private func initializeSubmodules(paths: [String] = []) {
        ...
        initializeTask = Task.detached(priority: .userInitiated) {
            do {
                ...
                await MainActor.run {
                    initializeTask = nil
                    message = paths.isEmpty ? GitSubmodulePluginLocalization.string("Submodule initialized") : GitSubmodulePluginLocalization.string("Initialized %@", paths[0])
                    errorMessage = nil
                    refresh()
                }
            } catch {
    ...
    private func updateSubmodules(paths: [String] = []) {
        ...
        updateTask = Task.detached(priority: .userInitiated) {
            do {
                ...
                await MainActor.run {
                    updateTask = nil
                    message = paths.isEmpty ? GitSubmodulePluginLocalization.string("Submodules updated") : GitSubmodulePluginLocalization.string("Updated %@", paths[0])
                    errorMessage = nil
                    refresh()
                }
            } catch {
