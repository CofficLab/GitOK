...
struct AutoPushConfigView: View {
    ...
    private func handleToggle(project: AutoPushProjectSnapshot, enabled: Bool) {
        ...
        showStatusMessage(String(format: GitAutoPushPluginLocalization.string("Auto-push %@: %@/%@"), enabled ? "enabled" : "disabled", project.projectTitle, branchName))
    }
    ...
    private func handleToggleConfig(_ config: ProjectBranchAutoPushConfig) {
        ...
        showStatusMessage(String(format: GitAutoPushPluginLocalization.string("Auto-push %@: %@/%@"), newStatus ? "enabled" : "disabled", config.projectTitle, config.branchName))
    }
    ...
    private func handleDeleteConfig(_ config: ProjectBranchAutoPushConfig) {
        ...
        showStatusMessage(String(format: GitAutoPushPluginLocalization.string("Configuration deleted: %@/%@"), config.projectTitle, config.branchName))
    }
    ...
    private func performPush(projectPath: String, branchName: String) {
        isLoading = true
        statusMessage = GitAutoPushPluginLocalization.string("Pushing...")
        ...
        Task {
            ...
            case .success:
                statusMessage = GitAutoPushPluginLocalization.string("Push succeeded")
            case let .failed(message):
                statusMessage = String(format: GitAutoPushPluginLocalization.string("Push failed: %@"), message)
            case .idle, .pushing:
                statusMessage = nil
            }
        }
