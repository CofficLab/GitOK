@testable import GitConflictResolverPlugin
import Foundation
import GitCoreKit
import GitOKCoreKit
import SwiftUI
...
@Suite("GitConflictResolverPlugin")
struct GitConflictResolverPluginTests {
    @Test("metadata matches legacy plugin identity")
    func metadata() {
        #expect(GitConflictResolverPlugin.metadata.id == "GitConflictResolverPlugin")
        #expect(GitConflictResolverPlugin.metadata.iconName == "exclamationmark.triangle")
        #expect(GitConflictResolverPlugin.metadata.tableName == "Localizable")
    }

    @Test("localized strings resolve")
    func localizedStringsResolve() {
        #expect(GitConflictResolverPlugin.metadata.displayName.isEmpty == false)
        #expect(GitConflictResolverPlugin.metadata.description.isEmpty == false)
    }
    ...
    func statusBarTrailingView() {
        ...
        #expect(!GitConflictResolverPlugin.statusBarTrailingItems(context: context).isEmpty)
    }
    ...
    func rootView() {
        ...
        #expect(GitConflictResolverPlugin.rootOverlay(context: context, content: root) != nil)
    }
