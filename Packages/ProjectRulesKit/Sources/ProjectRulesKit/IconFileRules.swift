...
public enum IconFileRules {
    public static let supportedFormats = ["png", "svg", "jpg", "jpeg", "gif", "webp"]
    ...
    public static func imageFileURLs(in directory: URL, entries: [String]) -> [URL] {
    ...
    public static func iconCount(in entries: [String]) -> Int {
        entries.filter(isSupportedImageFile).count
    }

    public static func preferredLookupCandidates(for iconId: String) -> [String] {
        if iconId.contains(".") {
            return [iconId]
        }
