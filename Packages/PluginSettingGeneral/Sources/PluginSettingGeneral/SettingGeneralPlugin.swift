...
/// 设置 - 通用 插件
///
/// 在设置视图中注册「通用」入口（从 Lumi 复刻并删减：移除新手引导 /
/// 网站 / 更新分组），每行均为 `AppSettingRow`（图标 + 标题 + 描述 +
/// 右侧 `AppButton`）。
///
...
public final class SettingGeneralPlugin: SuperPlugin, SuperLog {
    nonisolated static let logger = Logger(subsystem: "com.coffic.gitok.plugin.setting-general", category: "SettingGeneral")
    public let id = "com.coffic.gitok.plugin.setting-general"
