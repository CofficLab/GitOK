...
public struct RootView<Content>: View, SuperEvent, SuperLog where Content: View {
    ...
    /// 应用提供者
    var appProvider: AppVM { kernel.resolveProvider(AppVM.self)! }

    /// 插件提供者
    var pluginProvider: PluginService { kernel.resolveProvider(PluginService.self)! }

    /// 主题提供者
    @ObservedObject private var themeProvider: AppThemeVM

    /// Git 数据提供者
    var git: DataVM { kernel.resolveProvider(DataVM.self)! }
    ...
    public init(
        ...
        guard let themeVM = kernel.resolveProvider(AppThemeVM.self),
              let projectVM = kernel.resolveProvider(ProjectVM.self) else {
            fatalError("GitOK kernel is missing AppThemeVM or ProjectVM")
        }
        _themeProvider = ObservedObject(wrappedValue: themeVM)
        _projectVM = ObservedObject(wrappedValue: projectVM)
    }
    ...
    private var hostedContent: some View {
        if pluginProvider.hasPlugins {
            pluginProvider.rootViewWrapper(context: pluginRootContext) {
                baseContent
            }
        } else {
            baseContent
...
extension View {
    ...
    public func inRootView(kernel: KernelCoreContainer) -> some View {
        RootView(kernel: kernel) {
            self
        }
    }

    /// Legacy preview/integration entry point. Application composition uses
