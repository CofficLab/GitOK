        ...
        Button(action: action) {
            ZStack(alignment: .leading) {
                ...
                Image(systemName: systemImage)
                    ...
                    .scaleEffect(isHovered && !isActive && motionPreference.allowsMotion ? GitOKMotion.hoverScale : 1.0)
            }
        }
        .buttonStyle(.plain)
        .help(label)
        .animation(GitOKMotion.enabled(GitOKMotion.selection, preference: motionPreference), value: isActive)
        .onHover { hovering in
            GitOKMotion.animate(GitOKMotion.enabled(GitOKMotion.hover, preference: motionPreference)) {
                isHovered = hovering
            }
        }
    }
