...
@MainActor
public final class GitOKUIThemeRegistry: ObservableObject {
    public static let shared = GitOKUIThemeRegistry()

    @Published public private(set) var catalog: ThemeCatalog?

    @Published public private(set) var chromeTheme: any GitOKAppChromeTheme = UnconfiguredChromeTheme()

    @Published public private(set) var uiTheme: any GitOKUITheme = GitOKDefaultTheme()

    public init() {}

    public var themes: [GitOKUIThemeContribution] {
        catalog?.themes ?? []
    }

    public var selectedContribution: GitOKUIThemeContribution? {
        catalog?.selected
    }
    ...
    public func replaceAll(_ contributions: [GitOKUIThemeContribution]) throws {
    ...
    private func sortedUnique(_ contributions: [GitOKUIThemeContribution]) throws -> [GitOKUIThemeContribution] {
    ...
    private func applySelection() throws {
        ...
        uiTheme = resolvedUI
        GitOKUIThemeStore.shared.setTheme(resolvedUI)
    }
}
