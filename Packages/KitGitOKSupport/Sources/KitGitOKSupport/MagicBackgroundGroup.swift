...
public struct MagicBackgroundGroup {
    public enum GradientName: String, CaseIterable, Sendable {
    ...
    static var gradientInfos: [GradientName: GradientInfo] {
        [
    ...
    public init(for name: String) {
        self.name = Self.gradientName(for: name)
    }
    ...
    public static func gradientName(for rawValue: String) -> GradientName {
        if let gradient = GradientName(rawValue: rawValue) {
            return gradient
        }

        // Older GitOK versions persisted the three picker positions as 1/2/3.
        switch rawValue {
        case "2": return .blue2purple_l2r
        case "3": return .orange2yellow_tl2br
        default: return .blue2cyan
        }
    }
}
