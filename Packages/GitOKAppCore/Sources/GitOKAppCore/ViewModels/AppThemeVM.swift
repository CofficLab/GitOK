...
public final class AppThemeVM: ObservableObject {
    ...
    private let pluginProvider: GitOKThemeContributionsProviding
    private let settings: AppAppearanceSettingsStore
    private var cancellables = Set<AnyCancellable>()

    @Published public private(set) var themes: [GitOKUIThemeContribution] = []

    @Published public var currentThemeId: String {
        didSet {
    ...
    public init(
        pluginProvider: GitOKThemeContributionsProviding,
    ...
    public var currentTheme: GitOKUIThemeContribution? {
        themes.first(where: { $0.id == currentThemeId }) ?? themes.first
    }

    public var activeChromeTheme: any GitOKAppChromeTheme {
    ...
    private static func sync(pluginProvider: GitOKThemeContributionsProviding, registry: GitOKUIThemeRegistry) {
        ...
        let contributions = pluginProvider.themeContributions()
        do {
...
public struct GitOKDefaultChromeTheme: GitOKAppChromeTheme {
    public let identifier = "gitok-default"
    public let displayName = "GitOK"
    public let compactName = "GitOK"
    public let description = "Balanced GitOK default theme"
    public let iconName = "app.badge"
