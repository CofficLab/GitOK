...
public struct RemoteRepositoryRowView: View {
    ...
    public var body: some View {
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                HStack {
                    ...
                    if isCurrentUpstreamRemote {
                        Text(PluginRemoteRepositoryLocalization.string("Upstream"))
            ...
            HStack(spacing: 8) {
                if let webLink {
                    iconButton("safari", help: PluginRemoteRepositoryLocalization.string("Open Remote Page")) {
                        NSWorkspace.shared.open(webLink.url)
                    }
                }

                iconButton("doc.on.doc", help: PluginRemoteRepositoryLocalization.string("Copy Remote URL")) {
                    NSPasteboard.general.clearContents()
                    NSPasteboard.general.setString(remote.url, forType: .string)
                }

                iconButton("pencil", help: PluginRemoteRepositoryLocalization.string("Edit")) {
                    onEdit(remote)
                }

                iconButton("trash", help: PluginRemoteRepositoryLocalization.string("Delete")) {
        ...
        .alert(PluginRemoteRepositoryLocalization.string("Confirm Delete"), isPresented: $showDeleteAlert) {
            Button(PluginRemoteRepositoryLocalization.string("Cancel"), role: .cancel) {}
            Button(PluginRemoteRepositoryLocalization.string("Delete"), role: .destructive) {
                onDelete(remote)
            }
        } message: {
