...
/// Commit graph 渲染（移植自旧版 PluginGitCommitList.CommitGraphView）。
///
/// 用 `Canvas` 绘制分支连线与节点：竖直 lane 线贯穿上下、parent 跨 lane
/// 时画贝塞尔曲线、当前 commit 画圆点节点。
public struct CommitGraphView: View {
    ...
    public init(row: CommitGraphLayoutRules.Row?) {
        self.row = row
    }
    ...
    private var graphWidth: CGFloat {
        let visibleLanes = max(row?.laneCount ?? 1, 1)
    ...
    private var actualLaneSpacing: CGFloat {
        guard let laneCount = row?.laneCount, laneCount > 1 else { return preferredLaneSpacing }
    ...
    private func color(for id: String) -> Color {
        ...
        return palette[value % palette.count].opacity(0.75)
    }
}
