...
struct FileList: View, SuperLog {
    ...
    private var fileListHost: some View {
        FileListHostView(
            project: vm.project,
            ...
            loadCommitFiles: loadCommitFiles(project:hash:),
            loadWorktreeFiles: loadWorktreeFiles(project:),
            loadStatusEntries: loadStatusEntries(project:),
            makeWorktreeFilesFromStatusEntries: makeWorktreeFiles(from:),
            addFiles: addFiles(project:paths:),
            unstageFiles: unstageFiles(project:paths:),
            discardFileChanges: discardFileChanges(project:path:),
            discardAllChanges: discardAllChanges(project:),
...
private extension FileList {
    func loadCommitFiles(project: Project, hash: String) async throws -> [GitDiffFile] {
        try await project.changedFilesDetail(in: hash)
    }

    func loadWorktreeFiles(project: Project) async throws -> [GitDiffFile] {
        try await project.untrackedFiles()
    }

    func loadStatusEntries(project: Project) async throws -> [GitStatusEntry] {
        try await project.lightweightStatusEntriesAsync()
    }

    func makeWorktreeFiles(from entries: [GitStatusEntry]) -> [GitDiffFile] {
        Project.diffFiles(from: entries)
    }

    func addFiles(project: Project, paths: [String]) async throws {
        try await project.addFilesAsync(paths)
    }

    func unstageFiles(project: Project, paths: [String]) async throws {
        try await project.unstageFilesAsync(paths)
    }

    func discardFileChanges(project: Project, path: String) async throws {
        try await project.discardFileChangesAsync(path)
    }

    func discardAllChanges(project: Project) async throws {
