...
grep -q 'create-designed-dmg.sh' "$RELEASE_WORKFLOW" || fail "designed DMG script missing from release workflow"
grep -q 'hdiutil create' "${ROOT}/.github/scripts/create-designed-dmg.sh" || fail "native DMG creation missing"
grep -q 'GitOK-dmg-background.png' "${ROOT}/.github/scripts/create-designed-dmg.sh" "$RELEASE_WORKFLOW" || fail "DMG background asset missing"
test -f "${ROOT}/.github/dmg/GitOK-dmg-background.png" || fail "DMG background asset not found"
