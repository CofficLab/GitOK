...
const api = {
  ...
  onCommandDoublePressed: (callback: (event: Electron.IpcRendererEvent) => void): (() => void) => {
  ...
  onWindowHiddenByCommand: (callback: (event: Electron.IpcRendererEvent) => void): (() => void) => {
  ...
  mcp: {
    // 启动MCP服务
    start: (): Promise<{ success: boolean; message: string }> => ipcRenderer.invoke("mcp:start"),
